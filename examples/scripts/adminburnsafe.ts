...
// @note This is a demo function, it is not part of the SDK
const generateTxDataBurnToken = async (catalogSafeSdk: Safe, inputTokenId: BigNumberish): Promise<string> => {
    // @ts-ignore next
...
const createSafeTx = async (catalogSafeSdk: Safe, transactionData: any): Promise<{safeTx: SafeTransaction, proposedTx: any}> => {
    ...
    const signature = await safeTx.signatures.get('0x723aae1ad495f0b804624bc507741c08f214fba2');
    // sign TX from env key signer
    const safeTxHash = await catalogSafeSdk.getTransactionHash(safeTx);
    const proposedTx = await safeService.proposeTx(catalogSafeSdk.getAddress(), safeTxHash, safeTx, signature!);
...
connectSafe().then(async (catalogSafeSdk: Safe) => {
    ...
    const txData = await generateTxDataBurnToken(catalogSafeSdk, '1').then(async (data: string ) => {
