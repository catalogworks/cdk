// reserveAuctionListingETH.test.ts
// Jest test for ReserveAuctionListingETH
...
describe('Zora V3 ReserveAuctionListingETH', () => {
  describe('constructor', () => {
    it('throws an error if no contract address is provided', () => {
      const wallet = Wallet.createRandom();

      expect(() => {
        new ReserveAuctionListingETH(wallet, 420);
    ...
    it('throws an error if chainId does not map to a deployed instance', () => {
      ...
      expect(() => {
        new ReserveAuctionListingETH(wallet, 69);
    ...
    it('sets the instance to readOnly if provider is specified', () => {
      const provider = new JsonRpcProvider();
      const reserveAuction = new ReserveAuctionListingETH(
        provider,
        4,
        zoraAddresses.rinkeby.reserveAuctionListingEth
      );
    ...
    it('initializes an instance with the checksummed address', () => {
      const wallet = Wallet.createRandom();
      const rinkebyAddress = zoraAddresses.rinkeby.reserveAuctionListingEth;
      const reserveAuction = new ReserveAuctionListingETH(wallet, 4);
  ...
  describe('contract functions', () => {
    ...
    describe('write functions', () => {
      let defaultAuctionData: AuctionInputDataListingETH;
      let defaultBidData: {
        amount: string;
      ...
      const setupTestActions = async (): Promise<{
        reserveAuction: ReserveAuctionListingETH;
        erc721: Contract;
        ...
        try {
          const reserveAuction = new ReserveAuctionListingETH(
            mainWallet,
            50,
            reserveAuctionConfig.reserveAuctionListingETH
          );
          expect(reserveAuction.readOnly).toBe(false);
      ...
      describe('createAuction', () => {
        ...
        // 04
        it('throws an error if the input listing fee recipient address is not valid', async () => {
          const reserveAuction = new ReserveAuctionListingETH(
            mainWallet,
            50,
            reserveAuctionConfig.reserveAuctionListingETH
        ...
        it('creates an auction', async () => {
          ...
          reserveAuction.contract.on(
            ...
            (
              ...
              tokenId: BigNumber,
              auction: AuctionStructListingETH
            ) => {
