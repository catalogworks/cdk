...
export async function setupCatalog(wallet: Wallet,): Promise<CatalogConfiguredAddresses> {

    // setup the catalog shared creator contract (cnft)
    const cnft = await (await (await new TD606__factory(wallet)).deploy())._deployed();
